...
sample_id = args[1]
amplicon_abundance_table <- read.table(file = paste0(sample_id, '_amplicon_abundance_table_w_primer_ids.tsv'), head = F, stringsAsFactors = F, sep = "\t", quote = "")
...
write.table(amplicon_abundance_table_agg, file = paste0(sample_id, "_primers_dist_table.tsv"), sep = "\t", quote = F, col.names = T, row.names = F)
png(paste0(sample_id, '_primers_dist.png'))
pie(labels = paste0(amplicon_abundance_table_agg$primer_id, '\n', amplicon_abundance_table_agg$seq_count), x = amplicon_abundance_table_agg$seq_count)
dev.off()
